...
- **<!-- N:chapters -->89<!-- /N --> 章正文，一条主线。**
  语法 → 竞赛基本功 → 数据结构 → 算法 → 专题，每章开头写清前置依赖，不是知识点的堆叠。

- **<!-- N:problems -->366<!-- /N --> 道真题，题解全部验证过。**
  每道题都跑过官方样例，并在判题机上提交拿到真实判定；
  少数几道 CPython 常数过不去的，会写清为什么必须换 PyPy3。

- **正文里印出来的代码本身能跑。**
  正文代码块被单独抽出来喂官方样例验证，跑不过的不许印在正文里。

## 目录一览

| 卷                       | 内容                                                                      | 章数                        |
| ------------------------ | ------------------------------------------------------------------------- | --------------------------- |
| **卷一 · 核心卷** | Python 语法、竞赛基本功、数据结构与基础算法。笔试面试要用的基本都在这一卷 | <!-- N:vol1 -->53<!-- /N --> |
| **卷二 · 提高卷** | 图论、动态规划、数学与字符串算法                                          | <!-- N:vol2 -->29<!-- /N --> |
| **卷三 · 竞赛卷** | 平衡树、树链剖分、CDQ 分治、整体二分这一类，省选难度                      | <!-- N:vol3 -->7<!-- /N -->  |
...
题目取自四套公开题单，登记在 [`data/_sources.json`](data/_sources.json)——
新增或下线题单只改那一份，索引、导航、下面那张表都会自动跟上。
**逐套的题号段、题数与验证情况见下一节那张表。**

判题分两种模式，`scripts/verify.py` 按题单自动切：
**ACM 模式**读 stdin 写 stdout，走管道比对；
**核心代码模式**实现指定函数、不读 stdin，由 `scripts/corerun.py` 在子进程里
按函数签名喂参、取返回值、比对结构（`{3,2,1}` 与 `[3,2,1]` 判等）。

> 题面原文与官方样例**不入库**（体量与版权），`data/_problems.json` 是题目元信息的发布快照。
> 站点构建只读这份快照；`gen_index.py` 与 `verify.py` 需要原始资料，在 clone 的检出里跑不了。
...
| 来源 | 题单 | 题号段 | 判题模式 | 题数 | 已写题解 | 通过样例 |
| --- | --- | --- | --- | --- | --- | --- |
| [牛客在线编程](https://www.nowcoder.com/exam/oj?questionJobId=10&subTabName=online_coding_page) | 笔试模板必刷 | BISHI1–147 | ACM（stdin） | 147 | 147 | 147 |
| 同上 | 输入输出练习 | PIO1–18 | ACM（stdin） | 18 | 18 | 18 |
| 同上 | 面试必刷TOP101 | BM1–101 | 核心代码 | 101 | 101 | 101 |
| [力扣（LeetCode 中国站）](https://leetcode.cn/studyplan/top-100-liked/) | 热题 100 | 题号即力扣真实题号，不连续 | 核心代码 | 100 | 100 | 100 |
| **合计** | — | — | — | **366** | **366** | **366** |
...
文件的文档字符串**就是**站内题解页的正文。

> **`verify.py` 在 clone 下来的检出里跑不了。** 它要读各题的官方样例，
> 而题面与样例不入库（体量与版权）。跑它只会得到一串 `NO_PROBLEM`。
> **改了题解想自测**：自己造几组输入手动跑一遍 `sol.py`，
> 并在 PR 里写清用例与预期输出，维护者会替你过一遍判题机。

能在 clone 里跑的自测有两个：

```bash
uv run python scripts/check_syntax.py     # 全库源码的 Python 3.9 语法闸门
uv run python scripts/check_prose.py      # 文风与结构校验（CI 跑的就是这条）
```
...
3. 本地跑一遍 `uv run mkdocs serve` 确认渲染正常；动过 `.py` 就跑 `uv run python scripts/check_syntax.py`；
...
写完 <!-- N:chapters -->89<!-- /N --> 章正文和 <!-- N:problems -->366<!-- /N --> 道题解花了不少时间，站点长期免费、无广告、无付费墙。
**点一个 Star 就是最好的支持。**
