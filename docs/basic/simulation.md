...
另外 `datetime` 不接受非法日期，而某些题恰好要求**判断日期是否合法**。
**自己写 12 行更可控。**
...
题解见 [`solutions/nowcoder/BISHI16/sol.py`](../solutions/BISHI16.md)。

### BISHI17　纸牌游戏（简单）

> Alex 和 Bob 各有 2 张牌，两回合，每回合各翻一张比大小（**严格大者**赢，相等无人得分）。
> 赢的回合更多者获胜。求 **Alex 获胜的翻牌顺序数量**。$t \le 10^4$。
> 题面见 [原题](https://www.nowcoder.com/practice/74f915e366094b19b94334de8b68e733)。
...
题解见 [`solutions/nowcoder/BISHI17/sol.py`](../solutions/BISHI17.md)。
...
题解见 [`solutions/nowcoder/BISHI19/sol.py`](../solutions/BISHI19.md)。
...
题解见 [`solutions/nowcoder/BISHI15/sol.py`](../solutions/BISHI15.md)。

### BISHI11　变幻莫测（简单）

> 两种操作：交换 $(X,Y)\to(Y,X)$；变换 $(X,Y)\to(X+Y, X-Y)$。
> 求使 $X = Y$ 的最少操作次数，无法实现输出 $-1$。$|X|,|Y| \le 100$。
> 题面见 [原题](https://www.nowcoder.com/practice/5e702b6c7c114e64b5db4650d0ce0051)。
...
题解见 [`solutions/nowcoder/BISHI11/sol.py`](../solutions/BISHI11.md)。
...
| 题 | 主要考点 | 主讲章节 |
| --- | --- | --- |
| BISHI10 小红的字符串修改 | 字符环上的距离 + 枚举起点 | [字符串处理](../string/basic.md) |
| BISHI12 元素方碑 | **不变量**（奇偶位和），排序会毁掉它 | [排序 §7](sorting.md) |
| BISHI13 九倍平方数 | 数字和模 9 的性质 | [基础数学与递推](../math/recurrence.md) |
| BISHI14 特殊的科学计数法 | 浮点与字符串解析 | [浮点与科学计数法](../toolkit/float.md) |
| BISHI18 多项式输出 | **纯输出格式模拟**，规则多达 5 条 | [多项式](../math/poly/basic.md) |
| BISHI20 回文日期 | 日期合法性 + 回文枚举 | [回文](../string/manacher.md) |
...
> 这个「枚举一半再对称构造」的思路，与 [构造](construction.md) 的套路②相通。

---

### 面试题单里的两道矩阵模拟题

**[BM98](../solutions/BM98.md)**（同一道题在另一处题库里编号 [LC54](../solutions/LC54.md)）按顺时针从外向内输出矩阵元素。
写法有两种，各有各的坑：

1. **四个边界收缩**：维护 `top / bottom / left / right` 四条线，
   走完一行或一列就把对应的边收进来。
   **每走完一条边都要立刻判一次「四条边界是否已经交错」**，
   否则非方阵在最后一圈会把中间那一行或一列输出两遍。
2. **转圈剥皮**：每次取出第一行，再把剩下的部分逆时针转 90 度，重复到空。
   在 Python 里这一版短得多——`matrix = [list(r) for r in zip(*matrix)][::-1]` 一行完成旋转，
   代价是每轮都新建一个矩阵，额外空间 $O(nm)$。规模小的时候值得用它换正确性。

**[BM99](../solutions/BM99.md)**把上面那行拆开来考：把 $n \times n$ 矩阵原地顺时针转 90 度。
原地做法是**先沿主对角线转置，再逐行反转**，两步都只用交换，额外空间 $O(1)$。
转置的循环必须写成 `for i in range(n): for j in range(i + 1, n)`——
从 `i + 1` 起步，否则每对元素会被交换两次，等于没转。
不要求原地时，`[list(r) for r in zip(*a)][::-1]` 更省事，
`zip(*a)` 的转置全程在 C 层。
