...
[运算符与位运算](../python/operators.md) 讲过。
...
> 这条坑在 [运算符与位运算](../python/operators.md) 已经强调过，
> 这里再说一次，因为它值得说两次。
...
> **目标环境是 Python 3.9，统一用 `bin(x).count("1")`。**
...
**比 `math.log2` 快且没有浮点误差**，见 [倍增](binary-lifting.md)。
...
而且常数只有 $1/30$。见 [背包问题](../dp/knapsack.md)。
...
| --- | --- |
| 一堆数里只有一个出现奇数次 | 全体异或 |
| 区间异或和 | 前缀异或，$X_r \oplus X_{l-1}$，见 [前缀和与差分](prefix-sum.md) |
| 交换两数不用临时变量 | `a ^= b; b ^= a; a ^= b`（Python 里直接 `a, b = b, a` 更好） |
| 最大异或对 | 01-Trie，见 [Trie字典树](../string/trie.md) |
...
**尼姆博弈的 SG 值也是异或**，见 [博弈论](../math/game/impartial.md)。
...
> 题面见 [原题](https://www.nowcoder.com/practice/bc4c7936f5ed42cbb9131b6f39aa272b)。
...
题解见 [`solutions/nowcoder/BISHI30/sol.py`](../solutions/BISHI30.md)。

### BISHI31　二进制不同位数（简单）

> 给 $1 \le m, n \le 10^9$，求两者二进制表示中对应位不同的位数。
> 题面见 [原题](https://www.nowcoder.com/practice/daf9032926614dab91ca624a7759a868)。
...
题解见 [`solutions/nowcoder/BISHI31/sol.py`](../solutions/BISHI31.md)。
...
题解见 [`solutions/nowcoder/BISHI32/sol.py`](../solutions/BISHI32.md)。

---

### BM52 数组中只出现一次的两个数字

§5 那张表的第一行写着「一堆数里只有一个出现奇数次 → 全体异或」。
这题把「一个」改成「两个」，做法要多一步**分组**：

1. 全体异或得到 $s = x \oplus y$，其中 $x, y$ 是两个只出现一次的数。
   因为 $x \ne y$，所以 $s \ne 0$；
2. 取 `lowbit(s)`（§2 那一节的 `s & -s`），它是 $x$ 与 $y$ **第一个不同的二进制位**；
3. 按这一位是 0 还是 1 把全部数分成两组——
   成对出现的数必然落进同一组，而 $x$ 与 $y$ 必然被分开；
4. 两组各自全体异或，得到的就是 $x$ 与 $y$。

时间 $O(n)$、额外空间 $O(1)$，且只用到本章前两节的两件工具。
输出要求按从小到大排，最后 `sorted` 一下即可。

### LC136 只出现一次的数字

[LC136](../solutions/LC136.md)是[BM52](../solutions/BM52.md)的简化版：其余元素都恰好出现两次，只有一个落单。
异或满足交换律与结合律，且 `x ^ x == 0`、`x ^ 0 == x`，
所以整段异或起来成对的自行抵消，剩下的就是答案，$O(n)$ 时间、$O(1)$ 空间。
BM52 有两个落单元素，才需要多做一步：先整体异或得到 `a ^ b`，
取它的 `lowbit` 把所有数分成两组——两个落单元素在这一位上必然不同，因而分属不同组——
各组再异或一次即可。
...
| --- | --- |
| 取第 $k$ 位 | `(x >> k) & 1` |
| 置 1 / 清 0 / 翻转第 $k$ 位 | <code>x &#124; (1&lt;&lt;k)</code> / `x & ~(1<<k)` / `x ^ (1<<k)` |
| 低 $k$ 位掩码 | `(1 << k) - 1` |
| lowbit | `x & (-x)` |
| 消去最低位的 1 | `x & (x - 1)` |
...
| 循环里带 `<<` | 检查是否无限膨胀，加提前退出 |
| 大整数当 bitset | **优势项**，<code>x &#124;= x &lt;&lt; w</code> 一行顶一个内层循环 |
