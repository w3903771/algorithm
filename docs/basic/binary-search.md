...
## 4　实数二分：固定迭代次数，不要用 `while r - l > eps`

C++ 里常见的写法是 `while (right - left > precision)`。
**在竞赛里更推荐固定迭代次数**：
...
还有一串「其实也是二分」的东西，值得记住：

| 形态 | 说明 | 章节 |
| --- | --- | --- |
| **二叉搜索树 / 线段树** | 每次砍掉一半区间 | [线段树](../ds/segment-tree.md) |
| **倍增** | 从大到小试跳，本质是二进制拆分 | [倍增](binary-lifting.md) |
| **LIS 的 $O(n\log n)$ 解法** | 在「各长度的最小结尾」数组上二分 | [线性DP](../dp/linear.md) |
| **高精度除法** | 二分商 | [高精度与大整数](../toolkit/bignum.md) |
| **整体二分 / CDQ** | 对答案分治 | [整体二分](../technique/parallel-binsearch.md) / [CDQ 分治](../technique/cdq.md) |
| 分数规划 | 二分比值再判定 | — |
...
> $c_i, m \le 5\times10^8$。
> 题面见 [原题](https://www.nowcoder.com/practice/b77ff162bbab446993913fd684489cde)。
...
- **用的是 §2 的写法二（求最大可行解）**，所以 `mid` 必须 `(lo + hi + 1) // 2`。
...
> 题面见 [原题](https://www.nowcoder.com/practice/2e27509b990d4d02a70c0f208f078cdf)。
...
- **全 `R` 时答案是 0，不是 1。** 题面写着「输出一个正整数」，
  但实测数据里存在全 `R` 的测试点，期望输出是 $0$——一次都不用施法。
  这类「输出描述与真实数据不符」并不罕见：
  **边界情形以数据为准，题面的措辞不能当成担保**。
  更麻烦的是这种错法只挂在一个分支上，二分主体完全正确，
  很容易误判成二分写错而去反复改主循环。
...
> 题面见 [原题](https://www.nowcoder.com/practice/aac1989d183d4e6daf6ee2e1ca62b50c)。
...
> 相对误差 $10^{-6}$ 内算对。
> 题面见 [原题](https://www.nowcoder.com/practice/4f96afe5dfe74dad88dbe419d33f9536)。
...
- **半径本身要不要二分？不需要。** 答案一定取在某个点的距离上（半径再小就少覆盖一个点），
  ...
  $O(n\log V)$ 更快也更精确。**这就是上面说的「验证方法足够优秀时就抛弃二分」。**

若真要二分，模板是 §4 的固定 100 次迭代版本，`check(r)` 统计 $d_i \le r^2$ 的权值和。
...
> 题面见 [原题](https://www.nowcoder.com/practice/e0f5370bf8bb4d7d9c89c832c30da460)。
...
> **剪枝④是本题的灵魂**：`cur == 0` 时，说明正在为一根新的大木棍挑第一段。
> 若挑最长的那段都失败了，那它必须出现在某根大木棍里、且哪里都放不下，
> 整个 $L$ 直接判死。这一条把搜索树砍掉了绝大部分。

搜索与剪枝的系统讲法见
[记忆化搜索与剪枝](../search/memoization.md)。

---

### 面试题单里的四道二分题

**[BM17](../solutions/BM17.md)** 是模板题：升序数组里找目标值的**第一次出现**下标。
按 §1 那张表，直接 `i = bisect_left(a, x)`，再判 `i < len(a) and a[i] == x`。
手写的话对应 §2 的「找第一个满足条件的位置」那种边界写法。

**[BM21](../solutions/BM21.md)**把有序性打了个折：数组是升序数组旋转之后的结果，
整体不再有序，但**任取一段总有一半是有序的**——二分仍然成立。
判据是拿 `a[mid]` 与 `a[r]` 比：小于则最小值在左半（含 `mid`），
大于则在右半，**相等时无法判断，只能 `r -= 1` 退一步**。
最后这一条让最坏复杂度退化成 $O(n)$（全是相同元素时），这是允许重复元素的必然代价。
拿 `a[mid]` 与 `a[l]` 比是常见错法：数组没被旋转时会走错分支。

**[BM19](../solutions/BM19.md)**是 §6「二分的隐身形态」的好例子——数组根本无序，
但题目保证相邻元素不等且两端视为负无穷，于是
「`a[mid] < a[mid+1]` 就往右、否则往左」这条规则每次都能保住一个峰值在区间内。
**二分要的从来不是有序，是「能一次排除一半」的单调判据。**

**[BM18](../solutions/BM18.md)**给的是每行升序、每列也升序的矩阵。
逐行 `bisect` 是 $O(n\log m)$，能过；
但从**右上角**出发的走法更漂亮：比目标大就左移一列，比目标小就下移一行，
每步排除一整行或一整列，$O(n+m)$。
右上角是唯一同时具备「向左变小、向下变大」两个方向的起点，左下角同理，
另外两个角都不行——**起点选错，这个做法就不成立**。

### 另一处题库里的七道二分题

**先把三道模板题归位。**
[LC35](../solutions/LC35.md)就是「第一个 $\ge$ 目标的位置」，即 `bisect_left` 的返回值；
[LC34](../solutions/LC34.md)是它的左右两问，
`bisect_left` 与 `bisect_right` 各调一次，后者减一即右端点，两问都不命中时返回 $[-1, -1]$。
这两题直接调标准库比手写循环短且不会写错边界。
[LC74](../solutions/LC74.md)的每行首元素大于上一行末元素，**整个矩阵按行展平就是一个有序序列**，
把下标 $t$ 映射成 $(t / n, t \bmod n)$ 之后就是一次普通二分，$O(\log(mn))$。

**[LC240](../solutions/LC240.md)：条件放宽一点，二分就不成立了。**
这里只保证每行递增、每列递增，行与行之间没有衔接，展平后并不有序。
可行的做法是**从右上角出发**：当前值大于目标就左移一列，小于目标就下移一行，
每步至少排除一行或一列，$O(m + n)$。
逐行二分是 $O(m \log n)$，在 $m$ 与 $n$ 同阶时反而更慢。
**同名的两道题差的就是这一句前提**，判据一变，解法整个换掉。

**[LC33](../solutions/LC33.md)与 [LC153](../solutions/LC153.md)：二分不需要整体有序。**
二分真正要求的是「能用一次比较把答案排除在半边之外」，而不是数组有序。
旋转数组被断点分成两段，各自有序：拿 `a[mid]` 与 `a[hi]` 比一下就知道断点在哪一半，
另一半必然整段有序，于是目标在不在那一段可以直接判断。
LC153 只找最小值更简单：`a[mid] > a[hi]` 说明断点在右半，否则在左半含 `mid`，$O(\log n)$。
有重复元素时 `a[mid] == a[hi]` 无从判断，只能把 `hi` 减一退化到 $O(n)$。

**[LC4](../solutions/LC4.md)：二分的对象不是数组下标，而是「切一刀的位置」。**
在较短的那个数组上二分「左半部分取多少个」，另一个数组取多少就随之定死；
一刀切合法的判据是「两侧交叉比较都不越界」，即左半的最大值不超过右半的最小值。
复杂度 $O(\log \min(m, n))$。
四条边界（某一侧取空、某一侧取满）要用正负无穷兜住，这是这题最容易写错的地方。
本章「二分的隐身形态」那一节说的就是这类题：**被二分的量往往不是输入本身。**
