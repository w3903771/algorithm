...
[自定义排序](../python/sorting.md) 讲透，本章不重复，只讲**算法**。
...
## 2　三种 $O(n^2)$ 排序的 Python 实现

对照 C++ 的经典写法逐一翻译。这些实现**只用来理解原理，实战中一行都不要写**。

### 冒泡排序

每一轮把最大的元素「冒」到末尾。C++ 的常见写法用一个 `bool` 标志提前退出，
...
## 3　归并排序：本章唯一必须掌握的手写排序

标准写法是：递归分成两半，各自排好，再线性归并。
...
树状数组写法见 [树状数组](../ds/fenwick.md)，
离散化见 [桶计数与离散化](discretization.md)，
分治的三步框架见 [分治的三步框架](divide-conquer.md)，进阶形态见 [CDQ 分治](../technique/cdq.md)。
...
C++ 里通行的写法是 **Hoare 双指针划分 + 随机基准**，这是防卡的标准姿势：
...
堆排序的标准流程是「建大根堆 → 反复把堆顶换到末尾」。Python 里
...
| 稳定性 | 稳定 | 可以做多趟排序，见[自定义排序 §7](../python/sorting.md) |
...
| 用途 | 典型题型 | 关键点 |
| --- | --- | --- |
| **让贪心成立** | 区间调度、任务调度、田忌赛马 | 排序键由交换论证推出，见 [贪心](greedy.md) |
| **让双指针成立** | 两数之和、相差不超过 $k$ | 有序后指针才单调，见 [双指针与滑动窗口](two-pointer.md) |
| **让二分成立** | 区间元素计数、第 $k$ 大 | `bisect` 要求有序，见 [二分](binary-search.md) |
| **离散化** | 值域大但个数少 | `sorted(set(a))` + `bisect`，见 [桶计数与离散化](discretization.md) |
...
> **「还需不需要原来的位置？」** 需要就排 `(值, 下标)` 元组，或者干脆别排。
...
- **值域是 $[-10^9, 10^9]$**，跨度 $2\times10^9$，**不能用计数排序**（桶开不下）。
  这一条决定了本题必须是比较排序，见 [桶计数与离散化](discretization.md)。
- **别对 `bytes` 直接排序**。`data[1:n+1]` 是 `bytes` 列表，`sorted` 会按字典序排，
  `"10" < "9"`、负号更是灾难。必须先 `int`。
- **输出用 `" ".join`**，$10^5$ 个数时比 `print(*a)` 明显快。

题解见 [`solutions/nowcoder/BISHI21/sol.py`](../solutions/BISHI21.md)。

### BISHI22 / BISHI23 / BISHI24 / BISHI25：`key` 的四种典型形态

这四题的完整讲解在 [自定义排序 §12](../python/sorting.md)，
...
完整代码与经典贪心策略的对照放在 [贪心 §6](greedy.md)，那里会讲
...
题解见 [`solutions/nowcoder/BISHI12/sol.py`](../solutions/BISHI12.md)。
...
给一组区间，合并所有重叠的区间。同一道题在另一处题库里编号 [LC56](../solutions/LC56.md)。

[BM89](../solutions/BM89.md)是 §6 那张表第一行「排序让贪心成立」的教科书案例：
**按左端点升序排完之后，只需一次线性扫描**——
维护当前正在合并的区间，新区间的左端若不超过当前右端就把右端取 $\max$，
否则把当前区间收下、另起一段。$O(n\log n)$，排序占主导。

不排序则做不到线性：任意两个区间都可能重叠，直接两两比较是 $O(n^2)$，
而且合并之后还可能产生新的重叠，得反复迭代。
**排序把「任意两个都要比」降成了「只需和前一个比」**，这正是排序作为预处理的价值。

一个容易忽略的边界：题面若规定端点相接（如 $[1,4]$ 与 $[4,5]$）也算重叠，
判断条件就要写成 $\le$；若规定不算，则是 $<$。**两种题面都存在，读清楚再动手。**
...
| --- | --- |
| 让贪心成立 | [贪心](greedy.md) |
| 让双指针成立 | [双指针与滑动窗口](two-pointer.md) |
| 让二分成立 | [二分](binary-search.md) |
| 离散化 | [桶计数与离散化](discretization.md) |
