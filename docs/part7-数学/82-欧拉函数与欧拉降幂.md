...
> **配套例题**：BISHI73 【模板】欧拉函数计算Ⅰ ‖ 朴素求值：试除法、BISHI75 阶幂
> **来源**：S3 day9 resource《数论》（邹雨恒）第 62–68 页、《数论》（李有为）第 35–47 页、
> 《数论与信息学竞赛》（胡渊鸣）第 23–26 页；day10《NOIP 组合数学、数论选讲》第 17–18 页
> **前置**：[80-数论基础](80-数论基础.md)、[81-快速幂与逆元](81-快速幂与逆元.md)
...
- 先算 $\varphi(p^q)$。$[1, p^q]$ 中与 $p^q$ **不**互质的数恰是 $p$ 的倍数，共 $p^{q-1}$ 个。
  ...
  （这正是胡渊鸣课件那道「关键问题是求出 $\varphi(p^q)$ 的值」的答案。）
...
> **定义**（胡渊鸣课件）：对于函数 $f$，如果对任意 $\gcd(a,b)=1$ 有 $f(ab)=f(a)f(b)$，
...
| $\mu(n)$ | 莫比乌斯函数（[83 章](83-整除分块与数论进阶.md)） | $a=1$ 时 $-1$；$a\ge2$ 时 $0$ |
| $d(n)$ | 约数个数 | $a+1$ |
...
筛表本身用 `bytearray` 切片赋值，几乎不耗时（见 [80 章](80-数论基础.md) 模板二）。

---

## 82.3　求 $\varphi(1..n)$：线性筛

day10《NOIP 组合数学、数论选讲》直接给了 C++ 代码：
...
> 对比 [80 章](80-数论基础.md) 的 `bytearray` 埃氏筛（$10^6$ 只要 0.002 秒），
...
| 性质 | 内容 | 说明 |
| --- | --- | --- |
| 积性 | $\gcd(a,b)=1 \Rightarrow \varphi(ab)=\varphi(a)\varphi(b)$ | 见 82.1 |
| 质数 | $\varphi(p)=p-1$ | |
...
> - $m$ 为大于 2 的奇数：由 82.4，$\varphi(m)$ 是**偶数**，于是下一步减半。
>
...
> 题面见 [BISHI73 原题（牛客）](https://www.nowcoder.com/practice/6a22f91ad3904c6cbd624ae5ff6a4eac)。
...
题解见 [`solutions/BISHI73.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI73.py)。

### BISHI75 阶幂（中等）

> 定义 $fp(n) = 1$（$n \le 1$）、$fp(n) = n^{fp(n-1)}$（$n \ge 2$）。
> 给定 $1 \le n \le 10^6$，求 $fp(n) \bmod (10^9+7)$。
> 题面见 [BISHI75 原题（牛客）](https://www.nowcoder.com/practice/da7a14c1a58b48bd80e63771b82e50c5)。
...
模数每递归一层就变成 $\varphi(m)$。由 82.6 的收敛性，
...
> 正推时你不知道下一层需要的是模哪个数的结果。必须从最外层往里剥。

题解见 [`solutions/BISHI75.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI75.py)。
...
| 「既约分数 / 可见格点」 | $\varphi$ 或莫比乌斯（[83 章](83-整除分块与数论进阶.md)） |
| 指数是幂塔 / 指数本身要取模 | **欧拉降幂**，注意 $b\ge\varphi(m)$ 的条件 |
| 模数不是质数但要求逆元 | $\gcd=1$ 时用 exgcd（[81 章](81-快速幂与逆元.md)） |
