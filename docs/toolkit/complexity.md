---
id: toolkit/complexity
title: 复杂度与 Python 性能
volume: 1
lang: py
---
...
| $n \le 10^5$ | $O(n \log n)$ | 排序、堆、线段树、二分 | ✅ 内置排序全在 C 层；手写线段树危险 |
...
| `a[i:j]` 切片 | $O(j-i)$ | 会复制 |
| `len(a)` | $O(1)$ | 长度是存好的 |
| `a.sort()` | $O(n \log n)$ | Timsort，C 实现，无 Python 层开销 |
...
[背包问题 §4](../dp/knapsack.md#单调队列优化bishi138-真正要用的解法)。
...
这是「让循环消失」思路的典型应用，详见 [数论基础](../math/number/basic.md)。
...
[`solutions/nowcoder/PIO7/sol.py`](../solutions/PIO7.md)、[`solutions/nowcoder/PIO9/sol.py`](../solutions/PIO9.md)。
...
但仍然需要理解它们的原理，因为面试会问，而且归并排序的分治思想是求逆序对的基础。
见 [排序](../basic/sorting.md)。
