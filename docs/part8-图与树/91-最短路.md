...
> **配套例题**：BISHI105 【模板】单源最短路Ⅰ ‖ 无权图：BFS、BISHI106 【模板】单源最短路Ⅲ ‖ 非负权图：Dijkstra、BISHI108 最优乘车、BISHI109 邮递员送信
> **来源**：S2 `dijkstra.cpp`；S3 day7《简单图论》第 104–146 页；S4 `模板.docx`「最短路：dijkstra / SPFA+SLF / floyd / SPFA 路径条数 / floyd 判最小环」
> **前置**：[90-图的表示与遍历](90-图的表示与遍历.md)、[35-优先队列与堆](../part3-数据结构/35-优先队列与堆.md)、[61-BFS广度优先搜索](../part5-搜索/61-BFS广度优先搜索.md)
    ...
    while dq:
        ...
        for v, w in adj[u]:
            ...
            if nd < dist[v]:
                dist[v] = nd
                if w:
                    dq.append((nd, v))       # 权 1 排队尾
                else:
                    dq.appendleft((nd, v))   # 权 0 插队首
    return dist
```
...
### 朴素 $O(n^2)$ 版（S2 `dijkstra.cpp` 的写法）

S2 的参考实现是邻接矩阵 + 每轮线性扫描找最小值：
...
> [93 章](93-拓扑排序与二分图.md) 的 DAG 上 DP。

---

## 91.6　Bellman-Ford 与负环
...
def floyd_min_cycle(w, n):
    ...
    for k in range(1, n + 1):
        ...
        # 此时 g[i][j] 只用到了编号 < k 的中转点，正是我们需要的
        for i in range(1, k):
            wik = wk[i]
            if wik >= INF:
                continue
...
>    也顺便避开了 $i = j$ 时权值出错的情况（S4 原注释就是这么写的）。
...
| **判负环** | 跑完看是否存在 `W[i][i] < 0` |
| **「重要的城市」**（S3 day7 例题） | 若存在 $i,j$ 使 $W[i][k]+W[k][j] = W[i][j]$ 且 $C[i][j] = C[i][k]\cdot C[k][j]$，则 $k$ 重要 |
...
| 技巧 | 什么时候用 | 例子 |
| --- | --- | --- |
| **反图** | 「所有点到某一个点的最短路」 | **BISHI109**；S3 day7 P2296 |
| **拆点** | 一个点有多种「状态」 | **BISHI108**（站台 / 车上）；S3 day7 Steam Roller（横向 / 纵向） |
...
> 时限：C/C++ 5 秒，其他语言 10 秒。
> 题面见 [BISHI106 原题（牛客）](https://www.nowcoder.com/practice/d7fafd4f3340439e90597532850257b5)。
> 题解见 [`solutions/BISHI106.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI106.py)（已用官方样例验证）。
...
3. **边权和最大 $2\times10^5 \times 10^9 = 2\times10^{14}$**，C++ 必须 `long long`，
   Python 的整数无限精度，白送（见 [22-高精度与大整数](../part2-竞赛基本功/22-高精度与大整数.md)）；
4. 起点自己输出 0。
...
> 题解见 [`solutions/BISHI108.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI108.py)（已用官方样例验证）。
...
**拆点建虚拟链**（91.9 讲的技巧），边数降到 $O(\sum s) \approx 1.5\times10^5$：
...
> 题解见 [`solutions/BISHI109.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI109.py)（已用官方样例验证）。
...
def build_csr(n, us, vs, ws):
    """把边表压成 CSR：返回 (start, to, wt)。O(n + m)。"""
    m = len(us)
    deg = [0] * (n + 2)
    for u in us:
        deg[u] += 1
