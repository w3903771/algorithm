...
def tree_diameter_dp(n, start, adj, wt=None, root=1):
    ...
    for u in reversed(order):
        ...
        for i in range(start[u], start[u + 1]):
            ...
            if v == par[u]:
                continue
            cand = down[v] + (1 if wt is None else wt[i])
...
于是 **LCA 变成了 RMQ**，可以用 ST 表做到 $O(1)$ 查询。
...
> 题解见 [`solutions/BISHI96.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI96.py)（已通过官方样例验证）。
...
> 题解见 [`solutions/BISHI124.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI124.py)（已通过官方样例验证）。
