...
下标公式里**只出现 `b` 和 `c`，不出现 `a`**，这不是笔误。
把 `f[i][j][k]` 摊平成一条直线，就是按 `i` 分成 `a` 段、每段内部再按 `j` 分成 `b` 段、
每小段装 `c` 个元素——**每一维的下标只需要知道「它后面还有多少个元素」，
不需要知道自己这一维有多长**：

```text
i = 0 ────────────────  i = 1 ────────────────  …  共 a 段，每段 b*c 个
[ j=0 | j=1 | … | j=b-1 ][ j=0 | j=1 | … ]           每个 j 块 c 个
  ↑ k = 0..c-1
```
...
```python
i * (b * c)     # 跳过前 i 个 i-块，每块 b*c 个元素
  + j * c       # 再跳过块内前 j 个 j-块，每块 c 个元素
  + k           # 最后在 j-块里偏移 k
```

把 `i * b * c + j * c + k` 提一次公因式 `c`，就得到代码里的
`(i * b + j) * c + k`——两者完全等价，后者少一次乘法。

`a` 只用来决定数组总长 `a * b * c`（也就是「一共有多少段」），
一旦 `i` 已经给定，前面有几段是确定的，`a` 就再也不影响偏移量了。
同理，二维的 `f[i][j]` 摊平后是 `i * m + j`，公式里只有列数 `m` 没有行数 `n`。

> **口诀：乘的是「后面各维长度的乘积」。** 越靠后的维度权重越小，
> 最后一维的权重恒为 1。写错的典型症状是取到的值整体错位，
> 一维长度取 `a` 还是 `c` 直接决定错位的周期。
...
它的好处是**形状和题面一一对应**，不需要算任何偏移量，$n$ 在 $10^3$ 量级时够快，
调试时也能随手 `print` 中间结果。**先把这个写对，再谈优化。**

`input()` 的代价是每次调用都要走一遍提示符逻辑并去掉行尾换行，
换成 `sys.stdin.readline` 能快 5–10 倍（[实测对照表](../toolkit/io.md#1-三种读入方式以及为什么必须区分)）。
提速只改读的方式，不改逻辑：

```python
import sys

input = sys.stdin.readline                  # ✅ 最省事的提速：只换一个名字，其余代码不动
n, m = map(int, input().split())
g = [list(map(int, input().split())) for _ in range(n)]
```

`sys.stdin.readline` 与 `input()` 有一处不同：**它保留行尾的 `\n`**。
上面这样后接 `.split()` 时没有影响（`split()` 把换行也当空白），
但直接拿整行当字符串用时必须补一个 `.rstrip("\n")`，
否则换行会跟着答案一起输出。见 [输入输出处理](../toolkit/io.md)。

再往上还有 `buffer.read()` 一次性读全部，比 `input()` 快 20–50 倍，
适合 $nm$ 到 $10^6$ 的规模。它把整份输入当成**一维 token 流**，
代价是要自己算每一行的起止：
...
这个下标公式和上面「压成一维」是同一件事：`2` 是前缀偏移量，`m` 是每行的元素个数。

**三者的取舍**：$n \le 10^3$ 用 `input()`，写着最稳；再大就换
`input = sys.stdin.readline`，一行改动换 5–10 倍；只有在读入确实成为瓶颈时才上
`buffer.read()`——它是三者里唯一需要自己算偏移的写法，也是唯一会算错的写法。
...
> ```python
> q = [start]
> head = 0                        # 游标：q[head] 就是队首，还没被处理
> while head < len(q):
>     u = q[head]; head += 1      # ← 这两句顶替 popleft()：读队首 + 队首前移
>     for v in adj[u]:
>         if not vis[v]:
>             vis[v] = True
>             q.append(v)         # 入队照旧是尾部追加，O(1)
> ```
>
> **被替换掉的是出队，不是入队。** `append` 对应的是 `deque` 的 `append`（入队），
> 这一侧本来就是 $O(1)$，没有改的必要；真正贵的是 `list.pop(0)` 那一侧的出队，
> 它要把后面 $n-1$ 个元素整体前移。游标写法不删元素，只把 `head` 加一，
> 于是出队也变成 $O(1)$——**代价是 `q` 只增不减，内存里留着全部已处理的元素**。
>
> 这个副作用往往正好是想要的：`q` 最后就是一条完整的 BFS 访问序列，
> 可以回头拿去还原路径或者反向递推，而 `deque` 出队之后那些元素就没了。
> 反过来说，元素多到内存吃紧时，就该老老实实用 `deque`。
