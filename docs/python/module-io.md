...
> 但把逻辑放进 `main()` 函数**必须做**——局部变量比全局变量快 20%–30%（见[语法与执行模型](syntax.md)）。
...
| `UnboundLocalError` | 读未赋值的局部变量 | 函数里写全局变量忘了 `global`（见[函数](function.md)） |
...
## 8　例题：PIO3 多组_A+B_EOF形式

<!-- CHAPTER-EXAMPLE-TABLE -->

> 每行两个整数 $a, b\ (1 \le a, b \le 10^9)$，**读取至文件末尾为止**，
> 每行输出 $a + b$。
> 题面见 [原题](https://www.nowcoder.com/practice/295063bf1bce4c2e819a8f18a5efcd20)。
...
[面向对象与迭代器生成器](oop-iterator.md#5-迭代器协议)），
读到 EOF 时迭代自然停止——**用不着异常**。

这就是 §6 的原则在起作用：**能用正常控制流表达的，不要用异常**。
...
完整题解：[`solutions/nowcoder/PIO3/sol.py`](../solutions/PIO3.md)
