...
> **能取模就取模**——说的是**每乘一步就取一次模**，而不是把整个乘积累出来再取模。
> 单看一句 `(a * b) % MOD`，`a * b` 当然是先完整乘完的，`%` 改变不了求值顺序；
> 这条建议真正针对的是连乘的循环：
>
> ```python
> # ❌ prod 一路膨胀：n 个 10^9 相乘就是约 30n 个二进制位
> prod = 1
> for x in a:
>     prod *= x
> print(prod % MOD)
>
> # ✅ 每轮结束 prod 都被压回 [0, MOD)，任何时刻的中间值都不超过 MOD²
> prod = 1
> for x in a:
>     prod = prod * x % MOD
> ```
>
> 两种写法结果一样，差别全在中间值的位数上：前者第 $i$ 次乘法要扫 $O(i)$ 位，
> 累计 $O(n^2)$；后者每次乘法的两个操作数都在 $10^9$ 量级，是常数时间。
>
> 如果操作数本身就可能远超 `MOD`（例如刚读进来还没处理的输入），
> 那就先各自取模再乘：`(a % MOD) * (b % MOD) % MOD`。
> 两个数已经落在 $[0, \text{MOD})$ 内时这两次取模是多余的，写 `a * b % MOD` 就够。
...
> 比手写的 Python 快速幂快一个数量级。详见 [快速幂与逆元](../math/number/inverse.md)。
...
2. 需要精确十进制时用 `decimal.Decimal`（见 [浮点与科学计数法](../toolkit/float.md)）。
...
> 拿 `is` 比较数字或字符串会产生「有时对有时错」的诡异行为，根源是两个优化：
>
> - **小整数缓存**：解释器启动时就把 $-5$ 到 $256$ 这些常用小整数各建好一个对象，
>   之后所有用到它们的地方共享同一份。所以 `a = 100; b = 100` 时 `a is b` 是 `True`，
>   换成 `1000` 就成了 `False`——两次分别建了新对象。
> - **字符串驻留**：源码里形如标识符的字符串字面量会被登记进一张全局表并共享，
>   但运行时拼出来的字符串通常不进这张表。
>
> 两者都是为了省内存和加快比较，**并不保证**哪些值一定被共享；
> 依赖它写判断，换个 Python 版本就可能翻车。判相等永远用 `==`。

浅拷贝的经典陷阱见 [列表](list.md#浅拷贝陷阱)。
...
> **`int(s)` 的位数上限**：出于安全考虑（超长数字串的进制转换是平方复杂度，
> 可被用来发起拒绝服务攻击），字符串转整数默认最多 4300 位。
> 这个限制在 3.11 引入，并回溯到了 3.9.14、3.10.7 等安全更新版本。
...
> [原题](https://www.nowcoder.com/practice/bc4c7936f5ed42cbb9131b6f39aa272b)）。
...
> 解析科学计数法表示的数（题面见
> [原题](https://www.nowcoder.com/practice/ca0962879d3b40d8bb598fb9a905ac7a)）。
...
[浮点与科学计数法 §6](../toolkit/float.md#bishi14-特殊的科学计数法简单)。
...
| 场景 | 写法 |
| --- | --- |
| 大整数 | 直接算，不需要高精度；连乘循环里每步取模 `prod = prod * x % MOD` |
| 向上取整 | `(a + b - 1) // b` 或 `-(-a // b)` |
