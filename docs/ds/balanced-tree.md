...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[集合与多重集合](multiset.md)、[树状数组](fenwick.md)、[优先队列与堆](heap.md)

## 1　这一章为什么存在

NOIP 复赛的模板清单里，「树」这一节常见这样两个条目：
...
| 目标 | 说明 |
| --- | --- |
| 补上模板清单的知识缺口 | BST 的插入/查询/删除与退化、Splay 的原理与均摊分析 |
...
> 这两件覆盖了 Splay API 清单里的全部 8 个操作。**

---

## 2　二叉排序树（BST）

**BST**（binary search tree，二叉排序树，也叫二叉搜索树）的**定义**：
每个节点的左子树所有键 $<$ 自己 $<$ 右子树所有键。
...
| **可排序序列 + 区间翻转 / 区间移动**（「文艺平衡树」） | ❌ **没有替代品** | 这是「区间翻转版」的用途 |
| 动态加边删边的树（LCT） | ❌ | Python 完全不现实 |

> **遇到「区间翻转 + 区间查询」的题，Python 选手的处理是**：
> 1. 先看 $n$ 有多小——$n \le 2\times10^3$ 时**直接 `list` 切片翻转**（`a[l:r] = a[l:r][::-1]`，
>    切片翻转是 C 层操作，$2\times10^3$ 次 $\times$ $2\times10^3$ 长度 = $4\times10^6$ 次 C 层拷贝，
>    实测 15 毫秒，CPython 3.11 / Windows）；
...
这是 BISHI4 / BISHI5 的**实际最优解**，也是最推荐的有序集合实现。
完整模板 `ValueSet` / `ValueMultiset` 在
[集合与多重集合](multiset.md)，这里只讲**为什么它能赢**。
...
> 两者互不干扰。对比 Splay 要在每个节点上维护 `num` 字段（「完全版」就多了这个），
> 这里一个 `dict` 就搞定。
...
**关键在 `kth` 的写法**。完整模板见
[树状数组 · 树状数组上的二分](fenwick.md#4-树状数组上的二分倍增)：
...
**典型用途**：Dijkstra 的懒删除（[最短路](../graph/shortest-path.md)）、
「合并果子」类贪心、对顶堆维护中位数。
...
> ✅ 题解见 [`solutions/nowcoder/BISHI5/sol.py`](../solutions/BISHI5.md)，**已通过官方样例验证**。
> 本题在 [集合与多重集合](multiset.md) 是主讲；
...
> **回到那份 Splay API 清单**，逐条对照本题解：
>
> | Splay 常用版 | 本题解的实现 | 复杂度 |
> | --- | --- | --- |
...
> ✅ 题解见 [`solutions/nowcoder/BISHI129/sol.py`](../solutions/BISHI129.md)，**已通过官方样例验证**。
> 本题在 [分块](block.md) 也有讲解（分块视角）；
> 这里从**「有序集合的分布式版本」**视角看。
...
| 想练 | 推荐题 | Python 可行性 |
| --- | --- | --- |
| 有序集合全套操作 | P3369【模板】普通平衡树 | ✅ **用值域位图 + 树状数组，比 Splay 快** |
| 区间翻转（文艺平衡树） | P3391【模板】文艺平衡树 | ❌ Python 不现实（$n \le 10^5$） |
| 动态第 $k$ 小 | P3834（主席树）/ P2617 | ⚠️ 整体二分，见 [整体二分](../technique/parallel-binsearch.md) |
| 区间小于计数 | P2801 教主的魔法 | ✅ 分块 + `bisect`，同 BISHI129 |
