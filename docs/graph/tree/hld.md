...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[树的基础与遍历](basic.md)、[LCA 与树上路径](lca.md)、[线段树](../../ds/segment-tree.md)、[倍增](../../basic/binary-lifting.md)

## 1　这一章为什么存在

树链剖分在各家模板清单里都是「树」一节最长的两个模板之一。
在 NOIP 复赛里，树剖是**处理树上路径修改/查询的唯一通用武器**。

但配套题单里没有树剖题。最接近的是 BISHI124（纯 **LCA**，lowest common ancestor，
最近公共祖先：两点在树上共同的祖先里最深的那个）和 BISHI126（纯序列区间操作）——
...
换句话说，「重儿子优先」不是为了让链变长好看，是为了**让轻边变稀**，
而跳链的代价只由轻边数决定。
...
> 见 [树的基础与遍历](basic.md) 的 DFS 序小节。

### Python 必须写迭代 DFS

$n = 5\times10^5$ 的链状树，递归深度就是 $5\times10^5$——**必然爆 C 栈**。
两遍 DFS 都要改成迭代。技巧和 [强连通分量](../scc.md) 一样，
但树剖有个更省事的写法：
...
> 比树剖快一个数量级。** 见 [前缀和与差分](../../basic/prefix-sum.md)。
...
> ℹ️ `solutions/nowcoder/BISHI124/sol.py` 用的是另一条路线——欧拉序 + 分块稀疏表，查询 $O(1)$——
> 已用 Python 3 通过判题机。下面这份树剖版由 `scripts/verify_docs.py`
> 用**官方样例**实测通过，未单独提交判题机；两者的取舍见本节末尾。
...
>
> 总计 6–8 秒，**在 6 秒限制下余量很小**。
> 缓解因素是 $2\log n = 38$ 次跳链只是最坏界：随机形态的树上重链很长，
> 实测每次询问只跳 3–5 次，只有链状/星状的极端数据才逼近上界。
>
> **稳妥的替代路线是「欧拉序 + 分块稀疏表」**：预处理线性，单次查询 $O(1)$，
> 把每次询问的几十次跳链换成两次 `min(切片)` 的 C 层扫描。
> `solutions/nowcoder/BISHI124/sol.py` 就是这么写的，已用 Python 3 通过判题机。
> 还有一条离线路线是 Tarjan 离线 LCA（配并查集，见
> [并查集](../../ds/dsu.md)，均摊近似常数），
> 代价是要把 $5\times10^5$ 个询问全存下来再一次性回答。
...
> ℹ️ `solutions/nowcoder/BISHI126/sol.py` 用的正是下面这套双树状数组，已用 Python 3 通过判题机。
> 下面这份为便于讲解，初始数组用 $n$ 次单点加建树；题解文件里换成了 $O(n)$ 建树，
> 少掉约 $1.9\times10^7$ 次循环。两份都由 `scripts/verify_docs.py` 用**官方样例**实测通过。
...
**在 Python 里这不是「也可以」，是「必须」**。用双树状数组（[树状数组 · 形态三](../../ds/fenwick.md)）。
...
但这是 Python 里唯一有希望的做法，`solutions/nowcoder/BISHI126/sol.py` 用 $O(n)$ 建树把第一行的
$1.9\times10^7$ 次省掉后已通过判题机。
...
> **约 150 秒，差了 15 倍。**
>
> **这就是 §8 那张表的来历：Python 的树剖路径操作，$n,q$ 上限大约是 $3\times10^4$。**
