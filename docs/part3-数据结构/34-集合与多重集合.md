...
> 题解 [`solutions/BISHI5.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI5.py) 用的就是后者。
...
class ValueSet:
    ...
    def prev(self, x):
        ...
        if s:
            bb = s.bit_length() - 1
            return bb * self.BITS + self.blk[bb].bit_length() - 1
        return -1
    ...
    def next(self, x):
        ...
        if s:
            ...
            v = self.blk[bb]
            return bb * self.BITS + (v & -v).bit_length() - 1
        return -1
```
...
意味着不用离散化、不用平衡树，直接开值域数据结构。

题解用的正是 34.5 的两级位图：`BITS = 1024`，`NBLK = 10**6 // 1024 + 1 = 977` 块。
每次操作只做几次 128 字节大整数的位运算，总复杂度 $O(n)$。
...
题解见 [`solutions/BISHI4.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI4.py)。
...
题解见 [`solutions/BISHI5.py`](https://github.com/w3903771/algorithm/blob/main/solutions/BISHI5.py)。
