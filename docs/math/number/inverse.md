...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[数论基础](basic.md)、[位运算](../../basic/bit.md)

「模意义下的除法」值得从头铺一遍：

> 为什么模意义下的除法要放到最后讲？想一想：有理数允许分数的存在，模意义下还允许吗？
...
| --- | --- | --- |
| 整数乘法 | $a^b \bmod m$ | 本章 |
| **矩阵乘法** | $A^n$ | 线性递推加速（[基础数学与递推](../recurrence.md)） |
| 加法（快速乘） | $a \times b$ 用加法实现 | C++ 防溢出，见 §3 |
| 置换的复合 | 置换的 $k$ 次幂 | 求循环节 |
| $\min$/$\max$-plus 矩阵 | 最短路的 $k$ 步转移 | 图论 |

---

## 2　Python 取舍：内置 `pow` 不可替代
...
> C++ 写 `a ** b % m` 会立刻溢出得到垃圾值，错误当场暴露；
> Python 会**老老实实地把它算出来**，然后卡死。
> 参见 [高精度与大整数](../../toolkit/bignum.md)。
...
> Python 版本因此短得多——见 [整除分块与数论进阶](sqrt-decomposition.md)。
...
> 但是少了除法。仔细一想，前面只说了整数能对应到 $0 \ldots b-1$，
...
举例：$4$ 是 $3$ 在模 $11$ 意义下的逆元（$3\times4=12\equiv1$）；
$2$ 是 $7$ 在模 $13$ 意义下的逆元（$7\times2=14\equiv1$）。

### 逆元什么时候存在？

「什么时候找不到逆元？」这个问题的答案由裴蜀定理给出：
...
它被称为「比较不为人知的一种可以在 $O(n)$ 时间内完成此任务的莫名其妙的算法」。
...
组合数取模要先把**阶乘取模**表建出来，再对阶乘求逆元。阶乘表本身是一遍 $O(n)$ 的连乘取模，
真正的开销在逆元上——逐个求逆是 $n$ 次模幂，而下面这条倒推只要一次。
...
| **阶乘逆元倒推** | $O(n)$ + 1 次 pow | $P$ 质数，$P>n$ | **组合数题必用**（[组合数学](../combi/basic.md)） |
...
题解见 [`solutions/nowcoder/BISHI65/sol.py`](../../solutions/BISHI65.md)。
...
> 上面手写一遍是为了讲清原理——**模板题的意义在于说清内置函数在做什么**。

题解见 [`solutions/nowcoder/BISHI74/sol.py`](../../solutions/BISHI74.md)。
