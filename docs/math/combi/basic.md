...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[快速幂与逆元](../number/inverse.md)、[数论基础](../number/basic.md)
...
**用加法原理的前提是「互斥」**。如果两类方案有重叠，就要用**容斥**（§9）。
...
经典的「求和 II / 求和 III」两问问的就是杨辉三角**某一行**和**某条斜线**的和，
答案分别是上表的第 4 条和第 7 条。
...
那道答案写着「1024」的经典例题就是 $\sum_{k}\binom{10}k = 2^{10} = 1024$。

> 往后还有两条路：「将来会学到母函数（生成函数）」
> 「将来会在概率论中学到二项分布」。
...
[快速幂与逆元](../number/inverse.md) 的 81.6）：
...
> 见 [同余方程组与离散对数](../number/congruence.md)。
...
| --- | --- |
| 求**精确值**、$n$ 不大 | ✅ 最省事 |
| $n$ 大但 $k$ 很小（如 $n=10^6,k=7$） | ✅ 分子只有 40 多位，单次约 0.13 微秒 |
| $n=5\times10^5$、$k\approx n/2$、要取模 | ❌ 中间的大整数有 $1.5\times10^5$ 位，单次实测 1.34 秒 |
...
| 「$n+1$ 个数中必有两个互质」 | 相邻数对 |
| 「循环节长度有上界」 | 状态数有限（如斐波那契模 $p$，见 [基础数学与递推](../recurrence.md)） |
...
一道例题：「求 $1..n$ 中有多少个数是 2 的倍数或 3 的倍数」，
答案 $\lfloor n/2\rfloor+\lfloor n/3\rfloor-\lfloor n/6\rfloor$。

> loriex 还提醒了一句实话：
> 「这个常常和数论的乱七八糟的东西混到一起，结果是难度的骤升。」
> 莫比乌斯函数本质上就是容斥的系数——见 [整除分块与数论进阶](../number/sqrt-decomposition.md)。
...
> 一条经验之谈：打表看到这样的数列，兴许就是 Catalan 数。
...
$n$ 大时把 `pow(i+1, P-2, P)` 换成预处理的逆元表（[快速幂与逆元](../number/inverse.md) 模板），
否则 $n$ 次模幂会成为瓶颈。
...
> 题面见 [原题](https://www.nowcoder.com/practice/cd190da0f3614a8393c84981a11d024e)。
...
题解见 [`solutions/nowcoder/BISHI67/sol.py`](../../solutions/BISHI67.md)。

### BISHI69 [HNOI2008] 越狱（简单）

> $N \le 10^{12}$ 个房间排成一排，每人信仰 $M \le 10^8$ 种宗教之一。
> 若存在相邻两人信仰相同则「可能越狱」。求可能越狱的方案数，模 $P=100003$。
> 题面见 [原题](https://www.nowcoder.com/practice/33d5016548d84cbe8cdb08ca0c13c2f1)。
...
> **降幂的前提永远是底数与模数互质**，见
> [欧拉函数与欧拉降幂](../number/euler.md)。

题解见 [`solutions/nowcoder/BISHI69/sol.py`](../../solutions/BISHI69.md)。

### BISHI70 【模板】组合数（中等）

> $T \le 10^5$ 组，每组给 $0 \le n \le m \le 5\times10^5$，求 $\binom mn \bmod (10^9+7)$。
> 题面见 [原题](https://www.nowcoder.com/practice/53f82db091b2452bae5fef4437ffd0bb)。
...
题解见 [`solutions/nowcoder/BISHI70/sol.py`](../../solutions/BISHI70.md)。

### BISHI71 人员分组问题（中等）

> 从 $n \le 10^6$ 人中选出**恰好 5～7 人**的小组，求方案数模 $10^9+7$。
> 题面见 [原题](https://www.nowcoder.com/practice/482f31bf028f46aa9b7882a4b921ffcf)。
...
题解见 [`solutions/nowcoder/BISHI71/sol.py`](../../solutions/BISHI71.md)。

### BISHI66 子数列求积（中等）

> $n, q \le 10^5$，序列 $1\le a_i < 10^9+7$。$q$ 次询问区间乘积 $\prod_{i=l}^{r}a_i \bmod (10^9+7)$，
> **输出一行 $q$ 个数、空格分隔**。
> 题面见 [原题](https://www.nowcoder.com/practice/5daab034da954f5697dcf96c1808d34f)。

> **说明**：大纲把这题排在组合数学章，但它真正的考点是
> [快速幂与逆元](../number/inverse.md) 的**批量求逆**。
...
题解见 [`solutions/nowcoder/BISHI66/sol.py`](../../solutions/BISHI66.md)。
