...
> 这和 [高精度与大整数 §2](../../toolkit/bignum.md) 的准则一是同一回事。
...
更大的规模需要 FFT/NTT——但这已超出笔试范围（属省选内容），
这里不展开，FFT/NTT 是全书的明确排除项。
...
**笔试里的用法**：打表算出前几项，做差分，若差分几次就变成常数列，就说明有多项式通项，
可以直接拉格朗日插值或解方程求出来，避免推导。差分本身见
[前缀和与差分](../../basic/prefix-sum.md)。

---

## 5　例题：BISHI18 多项式输出（简单）
...
完整题解见 [`solutions/nowcoder/BISHI18/sol.py`](../../solutions/BISHI18.md)，已通过官方样例验证。
