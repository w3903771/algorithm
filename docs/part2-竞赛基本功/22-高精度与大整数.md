...
> **配套例题**：BISHI33 Poi 的新加法、BISHI63 计算阶乘
> **来源**：S4 `模板.docx` 的高精度加减乘除四个模板
...
对照 S4 模板文档里的四个 C++ 高精度模板（加、减、乘、除），Python 的等价物是：
...
Python 里几乎没有。唯一的例外是**题目要求你实现某种自定义的进位规则**——
这时「高精度」的形式还在，但语义变了。BISHI33 就是这样一道题。

作为教学，这里给出一份按 S4 模板结构组织的十进制高精度实现，
**仅供理解原理，实战中不要用**：
...
```python
print(int(input()) + int(input()))        # 这就是全部
```
...
> 一行搞定，而 C++ 选手要写高精度乘法。这就是 22.1 说的优势面。
...
def main():
    ...
    for _ in range(t):
        ...
        cur = int(a[0])
        for i in range(1, n):
            cur = (cur & int(a[i])) << 1      # f(x,y) = 2(x & y)
            if cur == 0:                       # 一旦归零，后面恒为零，提前退出
                break
        # Easy 版保证 l=1, r=n，询问行读掉即可
        for _ in range(q):
            p += 2
            out.append(cur)
    sys.stdout.write("\n".join(map(str, out)) + "\n")
...
> 这正是 22.2 说的陷阱：**Python 的大整数不会溢出，但会悄悄变慢**。
