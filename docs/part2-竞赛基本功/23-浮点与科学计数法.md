...
x > y + EPS                            # 严格大于
x < y - EPS                            # 严格小于
abs(x - y) < EPS * max(1.0, abs(x))    # 相对误差版，量级大时更稳
```
...
p = Fraction(1, 3) + Fraction(1, 6)     # Fraction(1, 2)
print(p.numerator, p.denominator)       # 1 2
...
mant = Decimal(s[0] + "." + s[1:4])             # 取够精度的前几位
r = mant.quantize(Decimal("0.0"), rounding=ROUND_HALF_UP)
