...
- **模块只会被执行一次**。第二次 `import` 直接拿缓存（`sys.modules`），
...
> 也会把你的拼写错误（`NameError`）静默吞掉，让 bug 变得无法定位。
...
**看到 RE 不要慌，本地跑一遍样例，报错行号直接告诉你哪里错了。**
...
>
> （`python -O` 可以关闭断言，但 OJ 不会给你加这个参数。）
...
`with` 是**上下文管理器**：块结束时（包括抛异常时）自动调用 `f.close()`。
**永远用 `with`，不要手动 `open` / `close`**——异常发生时手动版会漏关文件。
...
只有 NOIP/NOI 系列的**文件 IO 赛制**要求读写指定文件名，
那时的标准写法是重定向：
