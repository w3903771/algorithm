...
> **配套例题**：BISHI21 【模板】排序、BISHI55 判断质数、BISHI63 计算阶乘
> **来源**：菜鸟教程 Python3 内置函数速查表
...
详见 [12-自定义排序](12-自定义排序.md)。这里只强调一点：
...
[10-条件与循环](10-条件与循环.md#优化一enumerate-而不是-rangelena)）。
...
详见 [81-快速幂与逆元](../part7-数学/81-快速幂与逆元.md)。
...
见 [10-条件与循环](10-条件与循环.md#104range-的三个参数)。核心两点：
...
- **必须用 `math.isqrt(n)`，不能用 `int(n ** 0.5)`**。
  后者走浮点，$n$ 接近 $10^{12}$ 时可能算出 `999999.9999` 被截断成 `999999`，
  正好漏掉唯一的那个因子 $10^6$，导致合数被判成质数：

  ```python
  n = 10 ** 12
  math.isqrt(n)                # 1000000        精确
  int(n ** 0.5)                # 1000000        这个碰巧对
  int((10**18) ** 0.5)         # 999999999      这个就错了
  ```

  `isqrt` 是纯整数算法（牛顿迭代），**永远精确**。这是本章「整数问题不用浮点」原则的典型。
...
更快的 Miller-Rabin 见 [83-整除分块与数论进阶](../part7-数学/83-整除分块与数论进阶.md)。

### BISHI63　计算阶乘（简单）

> 给 $T\ (1 \le T \le 10^3)$ 个正整数 $n\ (1 \le n \le 10^6)$，
> 每个输出 $n! \bmod (10^9+7)$。
> 题面见 [`sources/05-nowcoder/problems/BISHI63.md`](../../sources/05-nowcoder/problems/BISHI63.md)。
...
- 用 `itertools.accumulate` 可以把这个循环也搬到 C 层（见
  [14-标准库速查](14-标准库速查.md#145itertools)）：
