...
这是 Python 算法题的**第二大 TLE 来源**（第一是 `in list`，见 [03-运算符与位运算](03-运算符与位运算.md#35成员与身份运算符)）：
...
1. **多行注释**（本质是一个没被赋值的字符串表达式，见 [01-语法与执行模型](01-语法与执行模型.md#14注释)）。
...
> 题面见 [`sources/05-nowcoder/problems/PIO10.md`](../../sources/05-nowcoder/problems/PIO10.md)。
...
> 完整讨论见 [20-输入输出处理](../part2-竞赛基本功/20-输入输出处理.md#205字符串的读入)。
