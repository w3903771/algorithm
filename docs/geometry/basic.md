...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[浮点与科学计数法](../toolkit/float.md)、[二分](../basic/binary-search.md)

## 1　这一章为什么存在

竞赛数学里有一套固定的平面几何工具：
平面直角坐标系、直线的一般式 / 点斜式 / 截距式、点到点距离公式、点到直线距离公式、圆。
这套工具是竞赛几何的全部起点，但**147 道笔试题里没有一道纯计算几何题**。
...
而是**把上面那几个公式变成 Python 里可以直接抄的工具**，
并说清楚一个 Python 独有的机会与陷阱：**用 `complex` 当二维向量**。

---

## 2　平面直角坐标系与 Python 的 `complex`

起点是「一个点用有序数对 $(x, y)$ 表示」。在 Python 里有三种选择：
...
圆的方程是 $(x-a)^2 + (y-b)^2 = r^2$。竞赛里最常用的是**覆盖判定**：
...
它把实数问题降成了离散问题。见 [二分](../basic/binary-search.md)。
...
> ℹ️ `solutions/nowcoder/BISHI86/sol.py` 已存在，下面的代码与它一致，并由
...
> 题面见 [原题](https://www.nowcoder.com/practice/94648fd37a644c76adb2adb29c2a3037)。

> ✅ 题解见 [`solutions/nowcoder/BISHI15/sol.py`](../solutions/BISHI15.md)，**已通过官方样例验证**。
...
> 见 [模拟](../basic/simulation.md)、[BFS广度优先搜索](../search/bfs.md)。
