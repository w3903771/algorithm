...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[字典](../python/dict.md)、[字符串处理](basic.md)、[位运算](../basic/bit.md)
...
C++ 模板里常见的 `int next[MAX][26]` 就是这个：给每个节点**固定分配** $C$ 个槽。

```python
def trie_array_build(words, C=26, base=97):
    """扁平数组版 Trie：son[node * C + c] = 子节点编号，0 表示不存在。

    对应 C++ 的 int next[MAX][26]。0 号节点是根，所以「0 = 不存在」不会歧义。
...
> `ch` 数组 $6\times10^6$ 个槽，约 50 MB，可以接受。见 §4。
...
> 理由与 [图的表示与遍历 §5](../graph/basic.md) 完全相同。
...
异或的关键性质是**按位独立**（见 [位运算](../basic/bit.md)）：
...
| 最小异或生成树 | Borůvka + 01-Trie（见 [最小生成树 §7](../graph/mst.md)） |
...
| 用途 | 说明 |
| --- | --- |
| **AC 自动机（Aho-Corasick 自动机）** | Trie + 失配指针（KMP 的 $\pi$ 数组在树上的推广），一次扫文本匹配全部模式串。见 [字符串匹配 KMP §8](kmp.md) |
...
> 见 [前缀和与差分](../basic/prefix-sum.md) 与
...
> 题面见 [原题](https://www.nowcoder.com/practice/feed1cd7546a4901965751b9fbf5f8a1)。
> 题解见 [`solutions/nowcoder/BISHI93/sol.py`](../solutions/BISHI93.md)（已通过官方样例验证）。

标准 Trie 模板题：插入时沿途 `pre += 1`，查询时沿 $t$ 走到底输出 `pre`。
真正要做的判断是**选哪种实现**——总长 $10^6$ 意味着最坏 $10^6$ 个节点，
§2 的选型表直接给出答案：**单个扁平 `dict`**。
...
4. **不能对每个前缀切片**。总长 $10^6$ 可能集中在一个串上，
   切全部前缀是 $5\times10^{11}$ 次字符拷贝（见 §1 的警告框）；
...
[LC208](../solutions/LC208.md)要求实现插入、精确查找与前缀查找三个接口，
正是上面模板的最小可用形态。
两条实现上的取舍值得单独点出来：

- **节点用数组还是用字典。** 只含小写字母时每个节点开一个长度 26 的数组，
  查子节点是一次下标访问；改用字典则省掉稀疏节点上的空槽，
  代价是每次多一次哈希。插入与查询都是 $O(L)$，$L$ 是串长，与词数无关。
- **「是不是一个完整的词」与「是不是一条前缀」必须分开记。**
  前者靠节点上的一个布尔标记，后者只要求走得到该节点。
  漏掉那个标记，`search("app")` 在只插入过 `"apple"` 时会错判为真。

上面「用 `set` 代替 Trie」那一节的结论在这里同样成立：
若只需要精确查找，哈希集合更省事；
**Trie 的价值全在前缀查询上**，本题把 `startsWith` 单列成一个接口，正是为了逼出这一点。
...
| AC 自动机 | Trie + 失配指针，多模式串匹配（[字符串匹配 KMP](kmp.md)） |
...
| 「树上路径异或最大」 | 前缀异或转最大异或对 |
| 「多个模式串同时匹配文本」 | **AC 自动机**（[字符串匹配 KMP](kmp.md)） |
