...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[DFS深度优先搜索](dfs.md)、[函数](../python/function.md)、[标准库速查](../python/stdlib.md)
...
这一章讲的就是这两把刀。记忆化的本质可以一句话说清：

> DP 的原理，可以理解为「**丢弃冗余信息**」。
> 暴力往往会枚举很多信息，但是未必所有的信息都对解决问题有帮助。
> 只考虑有用的信息，往往就可以大幅降低复杂度。
...
状态一多，缓存不停淘汰再重算，复杂度直接退回指数级，而且**表面上看不出任何异常**——
...
| `list` | `tuple(x)` |
| `set` | `frozenset(x)` |
| 一组布尔标记 | **二进制 mask 整数**（最快，见 [位运算](../basic/bit.md)） |
...
> **加了 `lru_cache` 之后，有效递归深度只剩一半。**

配合 [DFS 深度优先搜索 §4](dfs.md#4-递归改迭代) 的物理栈数据（Windows 主线程约 2500 层、
64 MB 线程栈约 7 万层），结论是：
...
DLX 的暴力搜索流程里，第 2 步就是一个标准的可行性剪枝：
...
> 题面见 [原题](https://www.nowcoder.com/practice/211c9fa6ea984185a16c6005e9793736)。
> 题解见 [`solutions/nowcoder/BISHI90/sol.py`](../solutions/BISHI90.md)（已用官方样例验证）。
...
4. `F = [[ones] * (N + 1)]` 里 `ones` 被共享了 101 次——
   **只读的表可以共享，但绝不能就地修改**。这是 [列表](../python/list.md)
   讲的浅拷贝陷阱的一个正面利用。
...
> 题面见 [原题](https://www.nowcoder.com/practice/b002b8eb564245fdbb8a02db8dcf03e4)。
> 题解见 [`solutions/nowcoder/BISHI79/sol.py`](../solutions/BISHI79.md)（已用官方样例验证，用的是递推版）。

[DFS 深度优先搜索](dfs.md#bishi79-取数游戏中等) 给了自底向上的递推版本。
...
> **对照 [DFS 深度优先搜索](dfs.md#bishi79-取数游戏中等) 的递推版**：
> 两份代码算的是同一张表 `dp[r][mask]`，
> 记忆化版**不需要想「先算哪一行」**（递归自动处理依赖），
> 递推版**不需要担心递归深度**且更快。
> 这题规模小，两种都行；规模一大，就只有递推能活。

### 搜索还是状压 DP：BISHI79 给的选型判据

**官方标签写着 dfs，但正解不是 dfs。** 逐格决策是 $2^{36} \approx 7\times10^{10}$ 个状态，
加剪枝也救不回来。真正起作用的是题目的结构：

> **八连通相邻只会发生在「同一行相邻列」和「相邻两行且列号差 $\le 1$」，
> 隔一行以上的两个格子永远不相邻。**

于是「本行选了哪些列」这个 $M$ 位 mask **完整概括了历史**——
第 $r+1$ 行的合法性只依赖第 $r$ 行的 mask。这就是无后效性，可以逐行递推。

| 做法 | 状态数 | Python 判断 |
| --- | --- | --- |
| 逐格 DFS + 剪枝 | $\le 2^{36}$ | ❌ |
| 逐行状压（本节的记忆化版 / [DFS 深度优先搜索](dfs.md#bishi79-取数游戏中等) 的递推版） | $N \times 2^M \le 6\times64$ | ✅ |

> **选型口诀**：**「决策只依赖上一行 / 上一层」→ 状压 DP，不要写搜索。**
> 搜索（DFS、DLX）是为「必须给出具体方案」或「约束无法压成小 mask」的情形准备的；
> 只要问的是**方案总数或最优值**，就先想能不能把中间状态合并掉。
> 通用形态见 [状压 DP](../dp/state.md)，
> 「要方案总数还是要具体方案」的完整判据见 [精确覆盖与 Dancing Links](dlx.md)。
...
> 求最多能收集多少金币。
> 题面见 [原题](https://www.nowcoder.com/practice/bde230df252c4b41a059c9b06fcf65b6)。

> ℹ️ 本题的 `solutions/` 题解文件尚未编写。下面的代码已由
> `scripts/verify_docs.py` 用**官方样例**实测通过，但未提交判题机。
...
（见 [DFS 深度优先搜索 §4](dfs.md#4-递归改迭代)），否则在 Windows 上直接静默崩溃。
...
给定矩阵，求最长的严格递增路径长度，路径每步只能走四邻域。
[BM61](../solutions/BM61.md)是 §1「把指数变成多项式」的标准案例：
朴素做法是从每个格子出发爆搜，指数级；
但**从某个格子出发的最长递增路径只取决于这个格子**，与怎么走到它的无关，
所以状态数只有 $nm$ 个，加上记忆化后每个状态只算一次，总复杂度 $O(nm)$。

这题不需要显式的访问标记——**严格递增本身就保证不会走回头路**，
不可能成环。这一点让它比一般的网格搜索干净：
记忆化搜索最怕「状态之间有环」，有环就不能靠递归求值，得改成拓扑序上的递推。
换个角度看，「把每个格子按值从小到大处理」就是这个隐含 DAG 的拓扑序，
那条路见 [DAG 上的 DP](../dp/dag.md)。

矩阵较大时递归深度可达 $nm$，按 §3 的建议改手写记忆化 ＋ 显式栈，
或者干脆按值排序后递推，两条路都不吃 C 栈。
