...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[队列与双端队列](../ds/queue.md)、[DFS深度优先搜索](dfs.md)
...
| 容器 | 栈（`list`） | **队列（`deque`）** |
| 递归形态 | 天然递归 | **天然迭代** |
| Python 爆栈风险 | ⚠️ 有（见 [DFS 深度优先搜索 §4](dfs.md#4-递归改迭代)） | ✅ **完全没有** |
| 无权最短路 | ❌ 不保证 | ✅ **保证** |
...
| $n$ 个开关的开/关 | 二进制 mask（[位运算](../basic/bit.md)） | `list`，长 $2^n$ |
...
> 题面见 [原题](https://www.nowcoder.com/practice/e88b41dc6e764b2893bc4221777ffe64)。
> 题解见 [`solutions/nowcoder/BISHI80/sol.py`](../solutions/BISHI80.md)（已用官方样例验证）。

网格 BFS 的标准模板题，§3 的技巧全部用上。
...
> 题面见 [原题](https://www.nowcoder.com/practice/33054daa2cc04fd6b97a0d18ccfc66a0)。
> 题解见 [`solutions/nowcoder/BISHI81/sol.py`](../solutions/BISHI81.md)（已用官方样例验证）。
...
> 问有多少个区域会被**完全**淹没。
> 题面见 [原题](https://www.nowcoder.com/practice/6d62436fda5f4ef997e68d1ce1dd6eb2)。
> 题解见 [`solutions/nowcoder/BISHI82/sol.py`](../solutions/BISHI82.md)（已用官方样例验证）。
...
> BISHI82 里不行。**写之前先问一句：后面还需要区分它们吗？**
...
> 题解见 [`solutions/nowcoder/BISHI83/sol.py`](../solutions/BISHI83.md)（已用官方样例验证）。

BFS 求最短路 + **前驱数组回溯路径**。
题目保证可行路径唯一，所以「最短路」和「那条唯一路径」是同一条。
（[DFS 深度优先搜索](dfs.md#bishi83-迷宫问题中等) 给了 DFS 显式栈的对照版本。）
...
> 题面见 [原题](https://www.nowcoder.com/practice/5a6f83a0e0214ba5a77f6cdc71a3027b)。
> 题解见 [`solutions/nowcoder/BISHI84/sol.py`](../solutions/BISHI84.md)（已用官方样例验证）。
...
> 题面见 [原题](https://www.nowcoder.com/practice/9dd512f784b24ece85c81600aa3bc06c)。
> 题解见 [`solutions/nowcoder/BISHI101/sol.py`](../solutions/BISHI101.md)（已用官方样例验证）。
...
> [树的基础与遍历](../graph/tree/basic.md)。

---

### BM57 岛屿数量

[BM57](../solutions/BM57.md)要在网格里数连通块，是 §3「网格 BFS」的原样形态：
外层双重循环扫格子，遇到未访问的陆地就把它入队、按四邻域扩散，
把整块染成已访问，扩散一次就给答案加一。

它比同名的 DFS 写法更适合 Python 的理由只有一个——**不吃递归深度**。
一块 $100 \times 100$ 的全陆地网格，DFS 递归深度可达 $10^4$，
默认递归上限之下会直接崩；BFS 的队列长度虽然也是同量级，但那是堆上的对象，不占 C 栈。
染色时**入队即标记**，不要等出队再标记，否则同一个格子会被重复入队，
队列长度可能膨胀到常数倍。这一条 §1 已经讲过，这题是它最常见的翻车点。

同一道题在另一处题库里编号 [LC200](../solutions/LC200.md)，两处的网格表示略有差异
（字符 `'1'`/`'0'` 与整数 1/0），判定逻辑一致。

### 另一处题库里的三道广搜题

这三道题把广搜的两项基本能力各用了一遍：**分层**与**多源同时出发**。

**[LC102](../solutions/LC102.md)：把「当前层有几个」在出队前记下来。**
广搜天然按层推进，但队列本身不带层号。
标准写法是每轮循环开始时先取一次队列长度，只弹这么多个，弹完的就是完整的一层。
让层号跟着节点一起入队也行，代价是每个节点多存一个整数。节点数为 $n$ 时都是 $O(n)$。

**[LC199](../solutions/LC199.md)：同一套分层，只取每层最后一个。**
按层遍历、每层取末尾元素即可；若改成先右后左入队，则取每层第一个。
容易写错的地方是把它当成「一路向右走」——
右子树为空而左子树还有更深的层时，那一层的最右节点其实在左子树里。

**[LC994](../solutions/LC994.md)：多源广搜，起点不止一个。**
把所有初始就腐烂的格子**一次性全部入队**，再按层扩散，层数就是所需分钟数。
逐个起点跑单源广搜再取最小值是 $O(k \cdot mn)$，多源一遍是 $O(mn)$——
多个源同时推进时，每个格子第一次被访问到的层数天然就是它到最近源点的距离。
扫完之后还要检查有没有残留的新鲜橘子，有则答案是 $-1$；
初始就没有新鲜橘子时答案是 $0$。这两处边界都要单独判。
...
| 看到什么 → **不要**用 BFS |
| --- |
| 边权是任意正数 → **Dijkstra**（[最短路](../graph/shortest-path.md)） |
| 要枚举所有方案 → **DFS 回溯**（[DFS 深度优先搜索](dfs.md)） |
| 树上后序 DP → DFS 两趟法 |
