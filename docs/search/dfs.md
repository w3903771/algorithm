...
<!-- CHAPTER-EXAMPLES -->
> **前置**：[函数](../python/function.md)（递归）、[栈](../ds/stack.md)、[复杂度与Python性能](../toolkit/complexity.md)
...
> 这是回溯题最高频的 bug，见 [列表](../python/list.md) 的复制陷阱。
...
**这是全书反复强调的取舍。** `itertools` 的循环在 C 层跑，
比手写 Python 递归快一个数量级：
...
| $2^n$ 子集（位运算枚举） | `for s in range(1 << n): ...` | 见 [位运算](../basic/bit.md) |
...
**并不会带来更多的栈空间**。每一层 Python 函数调用都要在 C 栈上放一个求值帧，
撞破 C 栈的结果是**进程直接被操作系统杀掉**：没有异常、没有 traceback、
在判题机上表现为「运行错误」甚至「输出为空」，极难定位。
...
详见 [复杂度与Python性能](../toolkit/complexity.md)。
...
def dfs_iterstack(s, adj):
    ...
    vis = {s}
    stack = [(s, iter(adj[s]))]             # 迭代器记住「这一层的循环走到哪了」
    while stack:
...
[记忆化搜索与剪枝](memoization.md)。

DLX（Dancing Links X，用双向十字链表实现的精确覆盖搜索）的搜索流程，就是「剪枝驱动的 DFS」的典范：
...
> 题面见 [原题](https://www.nowcoder.com/practice/0c8930e517444d04b426e9703d483ed4)。
> 题解见 [`solutions/nowcoder/BISHI76/sol.py`](../solutions/BISHI76.md)（已用官方样例验证）。
...
1. **$n = m = 1$ 时起点就是终点**，答案是 `Yes`。上面的代码在第一次 `pop` 时
   就会命中 `x == n-1 and y == m-1`，天然正确——但如果把判定写在「入栈时」
   就会漏掉起点；
2. 迷宫每行是**不含空格的字符串**，`split()` 按 token 读正好一行一个，
   不需要按行读；
3. **在 `bytes` 上索引得到的是 `int` 不是 `str`**，所以要拿 `ord('#')` 去比。
   这是 `bytes` 处理的通用注意点，见 [字符串处理](../string/basic.md)。

### BISHI77 数水坑（简单）

> $N \times M \le 100 \times 100$ 的田地，`W` 是水、`.` 是干地。
> **八连通**的水格属于同一个水坑。求水坑数量。
> 题面见 [原题](https://www.nowcoder.com/practice/664ca4289fcf457ba3109fdf4a7a1a05)。
> 题解见 [`solutions/nowcoder/BISHI77/sol.py`](../solutions/BISHI77.md)（已用官方样例验证）。
...
> 题解见 [`solutions/nowcoder/BISHI78/sol.py`](../solutions/BISHI78.md)（已用官方样例验证）。
...
> **手写回溯版**（§2 的 `permutations_dfs`）在 $n=9$ 时也能过
...
> 题解见 [`solutions/nowcoder/BISHI79/sol.py`](../solutions/BISHI79.md)（已用官方样例验证）。
...
| --- | --- |
| 行内不相邻 | `mask & (mask << 1) == 0` |
| 与上一行不八连通相邻 | <code>(m1 &#124; (m1 &lt;&lt; 1) &#124; (m1 &gt;&gt; 1)) &amp; m2 == 0</code> |
...
> 通常意味着**要按某一维压缩状态**，而不是真的要暴力搜。
> 详见 [状压 DP](../dp/state.md)。
...
> **保证可行路径存在且唯一**。
> 题面见 [原题](https://www.nowcoder.com/practice/cf24906056f4488c9ddb132f317e03bc)。
> 题解见 [`solutions/nowcoder/BISHI83/sol.py`](../solutions/BISHI83.md)（已用官方样例验证，用的是 BFS 版）。
...
六道题全落在 §2 的模板上，差别只在「候选集怎么选」和「怎么去重」。

**排列型。** [BM55](../solutions/BM55.md)是 `used[]` 模板的原样形态。
[BM56](../solutions/BM56.md)多一步**同层去重**：先排序，
然后在同一层里跳过「与前一个相同、且前一个尚未被使用」的候选。
`used[i-1] == False` 这个条件容易写反——它表示前一个相同值是在**本层**被撤销的，
说明这条分支已经枚举过了；若前一个仍在使用中，那是在**上层**选走的，属于合法的不同排列。
[BM58](../solutions/BM58.md)是同一份代码换个输出类型，去重逻辑一字不改。

**组合与剪枝。** [BM60](../solutions/BM60.md)（同一道题在另一处题库里编号 [LC22](../solutions/LC22.md)）
枚举长度 $2n$ 的括号串，候选只有两个：放左括号（还有剩就能放）、
放右括号（**已放的右括号数严格小于左括号数**才能放）。
把这两条写成 `ok()` 里的判断，生成的串天然全部合法，一个都不用事后过滤——
**这是剪枝的最佳形态：非法分支根本不进入搜索树。**

[BM59](../solutions/BM59.md)是回溯的压轴题。逐行放置，列与两条对角线各开一个标记数组
（对角线下标用 `r + c` 与 `r - c + n`），三个数组同时做「做选择 / 撤销选择」。
$n \le 9$ 时这份直白写法完全够用；把标记换成三个整数的位运算是常数优化，
思路见 [位运算](../basic/bit.md) 的子集枚举一节。

**泛洪填充。** BM57 岛屿数量（同一道题在另一处题库里编号 LC200）
是 §3 连通块的原样形态：扫一遍网格，遇到未访问的陆地就从它出发把整块染掉，
染色次数就是答案。用 DFS 还是 BFS 都行，
但 Python 里递归深度可能达到网格总格数，$10^4$ 以上就要按 §4 改成显式栈，
或者直接换成 [BFS 广度优先搜索](bfs.md) 的队列写法。

### 另一处题库里的七道回溯题

回溯题的差别几乎全落在**「下一层从哪里开始枚举」**这一个参数上，模板本身不变。
按那个参数分四组，组内的代码只剩三五行的出入。

**下一层从 0 开始：各层之间互不影响。**
[LC17](../solutions/LC17.md)每一层对应一个数字，枚举它映射到的三到四个字母，
深度等于输入长度 $L$，方案数最多 $4^L$；$L \le 4$ 时总量在 $256$ 以内。
[LC46](../solutions/LC46.md)与 BISHI78 是同一道题，每层枚举所有还没用过的数，靠一个布尔数组避免重取。

**下一层从 `i` 开始：同一个数可以重复取。**
[LC39](../solutions/LC39.md)允许同一个候选数被选任意多次，
所以进入下一层时起点仍是当前下标；起点写成 `i + 1` 就变成了「每个数最多用一次」，
**这一个字符就是两道不同题的分界**。
剪枝有两条：候选数先排序，一旦当前和加上候选数已经超过目标就整段停止枚举。

**下一层从 `i + 1` 开始：枚举子集与切分点。**
[LC78](../solutions/LC78.md)每一层做一次「取或不取」的选择，等价于起点单调右移，共 $2^n$ 个方案。
[LC131](../solutions/LC131.md)把「取到哪」当成切分点：枚举右端点，
判断这一段是不是回文，是就切下来递归剩下的部分。
逐次朴素判回文会让整体退化到 $O(n \cdot 2^n)$，
先用一遍 $O(n^2)$ 的区间动态规划把「$[i, j]$ 是不是回文」全预处理出来，每次判断降到 $O(1)$。

**在网格或棋盘上回溯：撤销比选择更容易写错。**
[LC79](../solutions/LC79.md)从每个格子出发四向搜索，走过的格子先标记、递归返回后**必须清掉标记**，
否则同一条路径上的格子会被后续分支误判成占用。
$m \times n$ 个起点、每步最多三个方向（来的方向不再走），
串长为 $L$ 时上界 $O(mn \cdot 3^L)$。
[LC51](../solutions/LC51.md)按行放子，列与两条对角线各用一个集合记占用，
对角线的键分别取 `r - c` 与 `r + c`；同样是「进入时加入、返回时移除」。
$n \le 9$ 时可行解不到 $400$ 个，搜索空间被这三个集合剪得很小。
...
| 看到什么 → **不要**用 DFS |
| --- |
| 「最少步数 / 最短路径」→ **BFS**（[BFS 广度优先搜索](bfs.md)） |
| 「所有方案数」且规模大 → **DP**（[DP 入门](../dp/basic.md)） |
