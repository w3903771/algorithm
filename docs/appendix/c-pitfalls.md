...
> 带 ⚠️ 的是**会导致 WA/TLE 但不报错**的坑——它们最危险，因为运行时看不出哪里错了。
> 每条都注明了详细讲解所在章节。
...
> 详见 [运算符与位运算 §5](../python/operators.md#5-成员与身份运算符)
...
实测十几倍差距。> 详见 [输入输出处理](../toolkit/io.md)
...
> 详见 [高精度与大整数 §2](../toolkit/bignum.md)
...
最彻底的是**改写成显式栈的迭代版本**。
> 详见 [DFS深度优先搜索](../search/dfs.md#4-递归改迭代)
...
> 详见 [浮点与科学计数法 §3](../toolkit/float.md)
...
> 详见 [列表 §浅拷贝陷阱](../python/list.md#浅拷贝陷阱)
...
题面写「若有多种答案，输出任意一个」时，输出和样例不同是**正常的**。
不要因此以为写错了。识别信号：
...
> 详见 [复杂度与Python性能 §4](../toolkit/complexity.md)

---

## C.7　版本兼容速查

全书代码一律兼容 **Python 3.9**。以下特性**不能用**（判题机版本未知时尤其要注意）：
