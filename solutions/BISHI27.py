...
坑在哪：
    ...
    4. 答案不唯一：符合四条约束的数对往往有很多组，题面也明说可以输出任意一组。
       本解法按「b 从小到大、a 取 b 的倍数从小到大」的顺序枚举，输出第一个命中的，
       所以 x = 10 时给出的是 "6 2" 而不是样例里的 "6 3"，两者都合法。
       这类题本地要用 special judge（特殊评测程序，按题目条件验证选手输出是否
       合法，而不是与标准答案逐字符比对）：本题配了 solutions/_spj/BISHI27.py，
       它逐条复查四个条件，并在输出 -1 时暴力确认确实无解。
"""
import sys
