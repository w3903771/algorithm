...
def main() -> None:
    # 一次性读入所有 token 再用游标走：操作 1 带一个参数，2/3/4 是单 token
    ...
    for _ in range(n):
        ...
        if op == b"1":
            ...
            # 判空看的是 head < len(q)：已出队的元素还留在列表里，
            # 用 len(q) > 0 判空会把已经出空的队列当成非空
            if head < len(q):
                head += 1                    # 出队不搬数据，只把队头指针挪一格
            else:
                out.append("ERR_CANNOT_POP")
        elif op == b"3":
            # 队首是 q[head] 而不是 q[0]；空队列的错误串与操作 2 的那个不一样
            out.append(q[head].decode() if head < len(q) else "ERR_CANNOT_QUERY")
        else:                                # 4：当前元素个数
            out.append(str(len(q) - head))   # 有效区间是 [head, len(q))
