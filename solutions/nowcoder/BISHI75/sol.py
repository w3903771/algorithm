...
def phi(x: int) -> int:
    ...
    # 用积性公式 φ(x) = x 乘上每个不同质因子的 (1 - 1/p)：
    # 只关心「有哪些质因子」，重数不影响结果，所以下面找到一个就把它除干净
    res = x
    p = 2
...
def calc(n: int, m: int) -> int:
    """fp(n) mod m。"""
    # 两个递归出口：模数沿 φ 链降到 1（约 60 层必到），或幂塔已经到顶
    if m == 1:
        return 0                    # 模 1 恒为 0，同时终止 φ 链
    if n <= 1:
        return 1                    # fp(0)=fp(1)=1，且此时 m >= 2
    # 扩展欧拉定理降幂：指数要在模 φ(m) 意义下算，所以每递归一层模数就换成 φ(m)。
    # 递归深度取决于 φ 链长（约 60）而不是 n，n = 1e6 也只递归几十层
    pm = phi(m)                     # 本层的新模数
