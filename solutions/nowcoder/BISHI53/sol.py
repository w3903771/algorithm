...
样例复核：
    ...
    Python 的任意精度整数见 docs/toolkit/bignum.md，
...
def main() -> None:
    ...
    n = int(data[0])
    a0 = int(data[1])                 # data[2] 是 b0，国王不领金币，b0 完全没用
    pairs = []
    # 相邻交换论证给出的排序规则是按 a_i * b_i 升序，键放在元组首位就能借默认排序
    for i in range(n):
    ...
    # 前缀乘积从国王的 a0 起步：他不参与排序、不领金币，但要计入乘积
    prefix = a0                       # 当前大臣前面所有人的左手乘积
    ans = 0
    # 排好序后一次扫描：先用「不含自己」的前缀算金币，再把自己的 a 并进去
    for _, a, b in pairs:
