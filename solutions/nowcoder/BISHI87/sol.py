...
这题考什么：
    **二分答案 + O(n) 判定**。直接贪心构造很容易漏情况，但「能否凑出 k 套」
    ...
    二分答案的通用套路见 docs/basic/binary-search.md。
...
def main() -> None:
    # 输入依次是 n、m 与紧随其后的 n 个 c_i，一次读完按下标切片取
    ...
    def ok(k: int) -> bool:
        ...
        # 判定要同时满足两条：Joker 总量够（need <= m），且每套至多用一张
        # Joker（need <= k）。只判前者，会在「Joker 很多但某一种牌极少」时超发
        need = 0
    ...
    # 开区间 (lo, hi) 每轮对半砍，上界约 1.3e10，35 轮左右就收拢到相邻两点
    while lo + 1 < hi:
        mid = (lo + hi) // 2
        if ok(mid):
