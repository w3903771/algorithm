...
这题考什么：
    ...
    均摊 O(1)、中间插入是 O(n)。见 docs/ds/array.md。
...
def main() -> None:
    # 一次性读进所有 token 再用游标解析：操作 2/5/6/7/8 只有一个数字、
    # 1/3 有两个、4 有三个，行长度不固定，按「每行两个数」读必然错位
    data = sys.stdin.buffer.read().split()
    q = int(data[0])
    i = 1                                # data[0] 是操作条数，参数从下标 1 开始
    a = []                               # 题目要维护的那个变长序列，就是一个 list
    out = []                             # 只有 3/7/8 有输出，攒起来最后一次写
    # 主循环：先取操作码，再由各分支自己把游标推过它带的参数
    for _ in range(q):
        op = data[i]
        i += 1
        if op == b"1":                       # 尾部追加
            ...
            i += 1
        elif op == b"4":                     # 在下标 p 与 p+1 之间插入
            # 插入位置是 p+1 而不是 p：insert(p, x) 会插到 p 的前面，
            # 样例 2 的 {5,3,7} 会得到 {5,4,3,7} 而不是正确的 {5,3,4,7}
            p = int(data[i])
            ...
            out.append(" ".join(map(str, a)))   # 一次拼成一行，不逐个 print
    # 全部结果攒够再一次 write，避免 7e3 次 print 的系统调用开销
    sys.stdout.write("\n".join(out) + "\n")
