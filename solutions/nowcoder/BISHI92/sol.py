...
def main() -> None:
    # T 可达 2e6 组、Σn 也是 2e6，IO 就是本题的主要开销：一次读完、一次写出。
    # 全程停在 bytes 上不做 decode，索引取到的是 int，比较比单字符 str 更快
    ...
    for _ in range(t):
        ...
        # 前缀函数递推。循环不变量：进入第 i 轮时 k = pi[i-1]，
        # 即「上一位已经匹配上的前缀长度」，本轮只能在它的基础上 +1 或回退
        pi = [0] * n
        k = 0                           # 当前候选的「最长相等真前后缀」长度
        for i in range(1, n):           # pi[0] 恒为 0：真前缀不能是整个串
            c = s[i]
            # k 是长度、pi 的下标是位置，两者差 1：长度 k 的前缀末位在 pi[k-1]。
            # 同理 s[k] 正是「候选前缀的下一个待匹配字符」，不用再加偏移
            while k and s[k] != c:      # 沿失配链回退，均摊 O(1)
                k = pi[k - 1]
            if s[k] == c:               # 对上了就把候选长度延长一位
                k += 1
            pi[i] = k                   # k 退到 0 仍不匹配时，pi[i] 自然是 0
        # 每组先拼成一整行攒着，全部算完再一次 write：2e6 个整数逐行 print 会慢几十倍
