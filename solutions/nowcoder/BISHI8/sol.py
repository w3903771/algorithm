...
坑在哪：
    ...
    6. 分块读入的两处拼接缺一不可：tail 处理「一个数字被块边界劈成两半」，
       carry 处理「x 在这一块、y 在下一块」。少了任何一个，都会在某个
       4MB 边界上把数字读错，而且错得很隐蔽——小数据完全测不出来。

样例复核：
    n = 3、操作 (1,5)、(2,4)、(1,7)：
    第 1 次 f(1) 旧值 0，贡献 0，随后 f(1)=5；
    第 2 次 f(2) 旧值 0，贡献 0，随后 f(2)=4；
    第 3 次 f(1) 旧值 5，贡献 3*5=15，随后 f(1)=7。
    合计 15，与样例一致。

前置章节：
    docs/python/dict.md、docs/ds/hash.md
"""
import sys
...
def main() -> None:
    # n 可达 5e6、输入上百 MB，read().split() 一把梭会一次性造出约 1e7 个
    # bytes 对象（每个至少 33 字节）直接 MLE，所以改成 4MB 分块流式处理
    read = sys.stdin.buffer.read
    f = {}                           # dict 就是题目要的映射 f，缺省 0 靠 get 补
    get = f.get                      # 绑成局部名，5e6 次查询省下属性查找
    total = 0                        # 中间和最多约 2^113，取模留到最后一次做
    idx = 0                          # 已处理的操作序号 i
    n = -1                           # 还没读到 n
    tail = b""                       # 上一块尾部未闭合的半个 token
    carry = None                     # 落单的 x（它的 y 在下一块）

    # 每轮吃一块 4MB：块内 split 出的 token 处理完就丢，
    # 内存峰值只取决于 dict 有多大，与输入总长无关
    while True:
        chunk = read(CHUNK)
        if not chunk:
            break
        if tail:
            chunk = tail + chunk     # 接上上一块被边界切断的半个数字
        toks = chunk.split()
        ...
        # tail 与 carry 是两种不同的跨块拼接，缺一不可：tail 管「一个数字被
        # 4MB 边界劈成两半」，carry 管「x 在这一块、它的 y 在下一块」
        ...
        # 每两个 token 是一对 (x, y)：先取旧值累加贡献，再把新值写回去，
        # 顺序颠倒就会拿赋值后的新值去乘，答案全错
        for j in range(0, len(toks), 2):
            x = int(toks[j])
            idx += 1                 # 操作序号从 1 开始计，乘的正是这个 i
            prev = get(x)
    ...
    # mod 2^64 就是无符号截断；全程都是非负数，不必担心 C 里的有符号溢出
