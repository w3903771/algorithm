...
这题考什么：
    ...
    位运算基础见 docs/basic/bit.md，
    离散化见 docs/basic/discretization.md，
    有序集合的其他实现见 docs/ds/multiset.md。
...
def main() -> None:
    # 按行切而不是按空白切：操作 4 那一行只有一个数字、其余行有两个，
    # 混成一条 token 流之后游标会从这里开始整体错位
    ...
    # 离散化：把值域压到 [0, m)，m <= 1e5，从此与原始值域（含负数）无关。
    # 前驱/后继的答案必然是某个插入过的值，而插入过的值都在 uniq 里，
    # 所以在下标上定位完再映射回原值，不会丢解
    uniq = sorted(set(vals))                     # 下标 -> 原值
    rank = {v: i for i, v in enumerate(uniq)}    # 原值 -> 下标
    nblk = len(uniq) // BITS + 1                 # 向上取整；m = 0 时也至少留一块

    # 两级位图：每块是一个 1024 位的大整数，summ 再记「哪些块非空」，
    # 于是本块内找不到时能一次跳到相邻的非空块，不必逐块扫过去
    blk = [0] * nblk             # blk[i] 的第 r 位 = 下标 i*1024+r 是否在集合中
    summ = 0                     # summ 的第 i 位 = 第 i 块是否非空
    size = 0
    out = []

    # 第二遍：全部操作都在下标值域上做，每条只走几次大整数位运算
    for op, x in ops:
        if op == 4:                                     # 集合大小
            out.append(str(size))
            continue

        # b = 块号，r = 块内位号，下面所有位运算都围绕这两个数展开
        i = rank[x]
        b, r = divmod(i, BITS)

        if op == 1:                                     # 插入
            # 已经在集合里就什么都不做，否则重复插入会把 size 算多
            if not (blk[b] >> r) & 1:
                blk[b] |= 1 << r
                summ |= 1 << b
                size += 1
        elif op == 2:                                   # 删除
            # 同理，删不存在的元素要静默忽略，不能把 size 减成负数
            if (blk[b] >> r) & 1:
                v = blk[b] & ~(1 << r)
                blk[b] = v
                if v == 0:
            ...
            # (1 << r) - 1 是低 r 位全 1 的掩码，与出来的恰好是「严格小于 x」的
            # 那些位；其中最高的一位就是最大的更小值，位号 = bit_length() - 1
            ...
            # 右移 r+1 位把 <= x 的部分整个丢掉，剩下的最低位（lowbit，v & -v）
            # 就是最小的更大值；移掉了 r+1 位，所以基准从下标 i+1 起算
