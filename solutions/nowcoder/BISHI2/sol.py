...
def main() -> None:
    # 一次性读入所有 token 再用游标走：push 后面跟一个参数，
    # pop/query/size 都是单 token，行长度不固定，按行读会错位
    data = sys.stdin.buffer.read().split()
    n = int(data[0])
    i = 1                                # data[0] 是操作条数，操作从下标 1 开始
    st = []                              # list 当栈用：append/pop/[-1] 全在尾部
    out = []
    for _ in range(n):
        op = data[i]
        i += 1
        if op == b"push":
            ...
            # 非空时只删不输出，空栈时才输出 Empty——这一条最容易写反
            if st:
                st.pop()
            else:
                out.append("Empty")
        elif op == b"query":
            # 只看栈顶不删除；元素是原样存下的 bytes，输出前 decode 回文本，负号自然保留
            out.append(st[-1].decode() if st else "Empty")
        else:                                # size
            out.append(str(len(st)))         # size 永远有输出，空栈时是 0
    # 攒够再一次 write，避免 1e5 次 print 的系统调用
    sys.stdout.write("\n".join(out) + "\n")
