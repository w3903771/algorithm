...
def main() -> None:
    # 按行切：操作 4 在样例里带一个占位参数（`4 0`），按行解析对
    # 「4」单独一行和「4 0」两种格式都成立，不会像 token 游标那样错位
    data = sys.stdin.buffer.read().split(b"\n")
    n = int(data[0])
    # 计数与有序性分家：多重性交给 cnt，值存不存在交给位图，
    # 于是删除时只有「计数从 1 掉到 0」才需要动位图
    cnt = {}                            # 值 -> 出现次数（只存 > 0 的）
    ...
    # 主循环：一行一条操作，data[0] 是行数，所以从下标 1 开始
    for k in range(1, n + 1):
        ...
        # 先平移到非负再定位：x 可能是负数，divmod 出负块号后
        # `1 << b` 会抛 ValueError: negative shift count。b 是块号，r 是块内位号
        x = int(p[1]) + OFFSET
        b, r = divmod(x, BITS)
        ...
        if op == b"1":                                  # 插入一个
            ...
            # 查的是「这个值出现几次」（可能是 0），与操作 4 的总个数不是一回事
